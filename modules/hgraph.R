...
hGraph <- function(nHypotheses = 4, # Number of hypotheses
  ...
  rr <- data.frame(
    x = radius * cos(radian), y = radius2 * sin(radian),
  ...
  x <- ggplot() +
    stat_ellipse(
      data = ellipses, aes(x = x, y = y, fill = fill, alpha = fillalpha, group = group),
      geom = "polygon"
    ) +
    ...
    geom_rect(
      data = transitionText, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax),
