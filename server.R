...
server <-function(input, output, session) {
  ...
  #-----R Hands On Table Outputs-----
  
  output$hotHypotheses <- renderRHandsontable({ # render user inputs for hypotheses
    DF <- dataHypotheses()
  ...
  output$hotGroups <- renderRHandsontable({ # render user inputs for hypotheses groups
    DF <- dataGroups()
    rhandsontable(DF, stretchH = "all", useTypes = FALSE, selectCallback = TRUE)
  })
  outputOptions(output, "hotGroups", suspendWhenHidden = FALSE)
  
  output$hotTransitions <- renderRHandsontable({ # render user inputs for transitions
    DF <- dataTrans()
    rhandsontable(DF, stretchH = "all", useTypes = FALSE, selectCallback = TRUE)
  })
  ...
  dataHypotheses <- reactive({
    ...
    isolate({
      if (is.null(input$hotHypotheses)) {
        DF = data.frame(Name  = c("H1", "H2", "H3", "H4"),
  ...
  #-----R Hands On Table Dependencies for Node Positioning----- 
