# Graph output
source("server/graph-output.R", local = TRUE)$value

# Graph matrix input buttons
source("server/graph-matrix-buttons.R", local = TRUE)$value

# Save & restore graph/code buttons
source("server/save-restore.R", local = TRUE)$value

# Display settings buttons
source("server/display-settings.R", local = TRUE)$value

# Example graphs button
source("server/show-example.R", local = TRUE)$value

# Save plot button
source("server/graph-save.R", local = TRUE)$value
